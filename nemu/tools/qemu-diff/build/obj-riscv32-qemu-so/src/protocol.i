...
extern __inline
    __attribute__((__gnu_inline__)) struct cmsghdr *__attribute__((__nothrow__,
                                                                   __leaf__))
  ...
  if ((unsigned char *)(__cmsg + 1) >
      ...
      ((unsigned char *)__cmsg + (((__cmsg->cmsg_len) + sizeof(size_t) - 1) &
                                  (size_t) ~(sizeof(size_t) - 1)) >
       ((unsigned char *)__mhdr->msg_control + __mhdr->msg_controllen)))
...
extern ssize_t __recv_chk(int __fd, void *__buf, size_t __n, size_t __buflen,
                          int __flags);
extern ssize_t __recv_alias(int __fd, void *__buf, size_t __n,
                            int __flags) __asm__(""
                                                 "recv");
extern ssize_t __recv_chk_warn(int __fd, void *__buf, size_t __n,
    ...
    __attribute__((
        __warning__("recv called with bigger length than size of destination "
                    "buffer")));

extern __inline __attribute__((__always_inline__))
__attribute__((__gnu_inline__)) __attribute__((__artificial__)) ssize_t
...
extern ssize_t __recvfrom_chk(int __fd, void *__restrict __buf, size_t __n,
                              ...
                              socklen_t *__restrict __addr_len);
extern ssize_t
__recvfrom_alias(int __fd, void *__restrict __buf, size_t __n, int __flags,
                 struct sockaddr *__restrict __addr,
                 socklen_t *__restrict __addr_len) __asm__(""
                                                           "recvfrom")

    ;
extern ssize_t
__recvfrom_chk_warn(int __fd, void *__restrict __buf, size_t __n,
                    size_t __buflen, int __flags,
                    struct sockaddr *__restrict __addr,
                    socklen_t *__restrict __addr_len) __asm__(""
                                                              "__recvfrom_chk")
    ...
    __attribute__((
        __warning__("recvfrom called with bigger length than size of "
                    "destination buffer")));
...
static uint8_t hex_nibble(uint8_t hex) {
  return ((*__ctype_b_loc())[(int)((hex))] & (unsigned short int)_ISdigit)
             ? hex - '0'
             : (__extension__({
                 int __res;
                 if (sizeof(hex) > 1) {
                   if (__builtin_constant_p(hex)) {
                     int __c = (hex);
                   ...
                   __res = (*__ctype_tolower_loc())[(int)(hex)];
                 __res;
               })) -
                   'a' + 10;
}
...
uint16_t gdb_decode_hex(uint8_t msb, uint8_t lsb) {
  if (!((*__ctype_b_loc())[(int)((msb))] & (unsigned short int)_ISxdigit) ||
      !((*__ctype_b_loc())[(int)((lsb))] & (unsigned short int)_ISxdigit))
    return (65535);
...
uint64_t gdb_decode_hex_str(uint8_t *bytes) {
  ...
  uint64_t weight = 1;
  while (
      ((*__ctype_b_loc())[(int)((bytes[0]))] & (unsigned short int)_ISxdigit) &&
      ((*__ctype_b_loc())[(int)((bytes[1]))] & (unsigned short int)_ISxdigit)) {
...
struct gdb_conn *gdb_begin_inet(const char *addr, uint16_t port) {
  ...
  socklen_t tmp;
  tmp = 1;
  int r = setsockopt(fd, 1, 9, (char *)&tmp, sizeof(tmp));
  if (r) {
    perror("setsockopt");
    ...
    ((void)sizeof((0) ? 1 : 0), __extension__({
       if (0)
         ...
         __assert_fail("0", "src/protocol.c", 118,
                       __extension__ __PRETTY_FUNCTION__);
     }));
  }
  tmp = 1;
  r = setsockopt(fd, IPPROTO_TCP, 1, (char *)&tmp, sizeof(tmp));
  if (r) {
    perror("setsockopt");
    ...
    ((void)sizeof((0) ? 1 : 0), __extension__({
       if (0)
         ...
         __assert_fail("0", "src/protocol.c", 124,
                       __extension__ __PRETTY_FUNCTION__);
     }));
  }
...
static uint8_t *recv_packet(FILE *in, size_t *ret_size, _Bool *ret_sum_ok) {
  size_t i = 0;
  size_t size = 4096;
...
const char *gdb_start_noack(struct gdb_conn *conn) {
  ...
  _Bool ok = size == 2 && !strcmp((const char *)reply, "OK");
  free(reply);
