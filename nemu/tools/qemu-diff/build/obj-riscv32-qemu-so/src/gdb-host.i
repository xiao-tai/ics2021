...
_Bool gdb_connect_qemu(int port) {

  while ((conn = gdb_begin_inet("127.0.0.1", port)) == ((void *)0)) {
    usleep(1);
  }
...
static _Bool gdb_memcpy_to_qemu_small(uint32_t dest, void *src, int len) {
  char *buf = malloc(len * 2 + 128);

  ((void)sizeof((buf != ((void *)0)) ? 1 : 0), __extension__({
     if (buf != ((void *)0))
       ;
     else
       __assert_fail("buf != NULL", "src/gdb-host.c", 16,
  ...
  _Bool ok = !strcmp((const char *)reply, "OK");
  free(reply);

  return ok;
}

_Bool gdb_memcpy_to_qemu(uint32_t dest, void *src, int len) {
  const int mtu = 1500;
...
_Bool gdb_getregs(union isa_gdb_regs *r) {
...
_Bool gdb_setregs(union isa_gdb_regs *r) {
  ...
  ((void)sizeof((buf != ((void *)0)) ? 1 : 0), __extension__({
     if (buf != ((void *)0))
       ...
       __assert_fail("buf != NULL", "src/gdb-host.c", 71,
                     __extension__ __PRETTY_FUNCTION__);
   }));
  buf[0] = 'G';
