...
void add_alarm_handle(alarm_handler_t h) {
  ...
  ((void)sizeof((idx < 8) ? 1 : 0), __extension__({
     if (idx < 8)
       ;
     else
       __assert_fail("idx < MAX_HANDLER", "src/device/alarm.c", 12,
...
void init_alarm() {
  struct sigaction s;
  memset(&s, 0, sizeof(s));
  s.__sigaction_handler.sa_handler = alarm_sig_handler;
  int ret = sigaction(26, &s, ((void *)0));
  do {
    if (!(ret == 0)) {
      ...
      assert_fail_msg();
      ((void)sizeof((ret == 0) ? 1 : 0), __extension__({
         if (ret == 0)
           ;
         else
           __assert_fail("ret == 0", "src/device/alarm.c", 28,
                         __extension__ __PRETTY_FUNCTION__);
       }));
    }
  } while (0);
  ...
  ret = setitimer(ITIMER_VIRTUAL, &it, ((void *)0));
  do {
    if (!(ret == 0)) {
      (fflush(stdout), fprintf(stderr, "\33[1;31m"
                                       "Can not set timer"
      ...
      ((void)sizeof((ret == 0) ? 1 : 0), __extension__({
         if (ret == 0)
           ...
           __assert_fail("ret == 0", "src/device/alarm.c", 35,
                         __extension__ __PRETTY_FUNCTION__);
       }));
    }
  } while (0);
